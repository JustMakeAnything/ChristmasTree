...
uint16_t sizeLed(uint8_t numberOfLeds){
	return (uint32_t)65536 * numberOfLeds / numLeds;
}
...
void mergeIntoLED(uint32_t baseColor,uint16_t currentLED, uint8_t color1Intensity, patternParams_S params){
	mergeIntoLED(baseColor,currentLED,color1Intensity,params.merge,params.gamma,params.strip);
}
...
void pattern(patternParams &params){
	...
	for(uint16_t currentLED = 0; currentLED < numLeds; currentLED++) {
		...
		switch (params.shape){
		case AllOn:
			mergeIntoLED(params.baseColor,currentLED,params.intensity,params);
			break;
		case Callback:
			mergeIntoLED(params.callback(currentLED,relativePos),currentLED,255,params);
			break;
		...
		case Solid:{
			if(relativePos<params.size || params.repeat == true){
				...
				mergeIntoLED(params.baseColor,currentLED,255,params);
			}
			break;
		}
		case SawRight:{ 
			...
			if(relativePos<params.size || params.repeat == true){
				relativePos %= params.size;
				mergeIntoLED(params.baseColor,currentLED,255-relativeToSize,params);
			}
			break;
		}
		case SawLeft:{
			...
			if(relativePos<params.size){
				mergeIntoLED(params.baseColor,currentLED,relativeToSize,params);
			}
			break;
		}
		case Sine:{
			...
			if(relativePos<params.size || params.repeat == true){
				...
				mergeIntoLED(params.baseColor,currentLED,strength,params);
			}
			break;
		}
		case SineFull:{
