...
//            LEDs[stripNumber] = Adafruit_NeoPixel(NumberLeds,  ledStripDataPin,  getPixelType());
            ...
            for (byte num = 0; num < getAmountOfLeds(); num++) {
                LEDs[stripNumber].setPixelColor(num+NumberLeds, 0, 0, 0);
            }
...
uint16_t getHue(uint8_t red,  uint8_t green,  uint8_t blue)  {
    ...
    sat = 255 * static_cast<float>(max - min) / value;
    if (sat == 0) {
        hue = 0;
        return 0;
    }
...
void pixelhue(uint8_t num,  uint16_t huein, uint16_t offset) {
    for (uint8_t stripNumber = 0; stripNumber < ledStripCount; stripNumber++) {
        ...
        #endif
            LEDs[stripNumber].setPixelColor(num,
                                            LEDs[stripNumber].gamma32(LEDs[0].ColorHSV(huein+offset*100,
                                            255, 255)));
        #ifdef FASTLED
        }
        #endif
...
void pixelcol(uint8_t strip, uint8_t num,  uint32_t color) {
    ...
    #endif
        uint8_t side = 0;
        // Side currently only for one strip
        if (ledStripCount == 1 && strip!=0){
            side=strip;
            strip=0;
        }
        switch (layout) {
        ...
        case layoutReverse:
            ...
            break;
        case layoutReverseUneven:
            // uneven amount of LEDs.
            if(side==0 || side==2){
                LEDs[strip].setPixelColor((NumberLeds << 1)-num, color);
            }
            break;
        default:
            break;
